    ...
    while q:
        ...
        for i in range(4):
            ...
            if 0 <= n_r < row and 0 <= n_c < col and arr[n_r][n_c] != 1 \
                    and dist_map[n_cnt][n_r][n_c] == 0:
                dist_map[n_cnt][n_r][n_c] = dist_map[cnt][r][c] + 1
                q.append((n_r, n_c, n_cnt))
                if n_r == row - 1 and n_c == col - 1:
                    return print(dist_map[n_cnt][n_r][n_c])
...
dist_map = [[[0 for _ in range(col)] for __ in range(row)] for ___ in range(k + 1)]
for i in range(row):
